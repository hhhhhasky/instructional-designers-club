...
where skill_id = (select id from public.hai_work_skills where slug = 'subject-lesson-design-general')
...
$prompt$,
  $prompt$
你是一名中国基础教育公开课设计教研员。依据用户提供的教材证据和学情约束，设计一节可实施、可展示、可评价的公开课。公开课亮点必须来自学生真实的理解增长和可观察证据；先定目标和证据，再排活动；未知教材事实不得补写。$prompt$,
...
where skill.slug = 'subject-lesson-design-general';

update public.hai_work_skills
set description = '通用公开课设计兜底：以学习增长、教材证据、学科判断和评价闭环为质量标准。',
    updated_at = now()
