...
-- Publish the repository copy of HAI Chat Skill v12 as one immutable
-- database snapshot. Abort if production no longer matches the downloaded
-- v11 baseline, so concurrent admin edits cannot be overwritten.
do $migration$
declare
  v_skill_id uuid;
  v_version_id uuid;
...
正式回答通常用一至三个短段落：先给判断，再给必要解释和一个动作、例子或追问。删除重复说法、无关背景和机械追问，不展示完整检查链、规则名或内部路由。最终回答可以使用 Markdown，并保留标题、段落、列表、引用、表格、粗体、斜体、行内代码和代码块等必要结构；不要为了追求口语化而删除 Markdown 标记或压平换行。
...
- 是否删掉了重复解释、无关背景和机械追问？
$skill_v12$;
  v_method_cards text := $method_cards_v12$# 35 张课程方法卡索引
...
$method_cards_v12$;
  v_evaluation_rubric text := $evaluation_rubric_v12$# HAI Chat 价值优先验收规则
...
begin
  ...
  if exists (
    ...
    where skill_id = v_skill_id and version_label = 'v12'
  ) then
    raise exception 'hai-consultation v12 已存在，不能覆盖不可变版本。';
  end if;
  ...
  if v_previous_version_id is null
    or v_previous_version_label <> 'v11'
    or v_previous_snapshot_hash <> 'bf6e8f005392754cc4384720f5c79ad9' then
    raise exception
      'hai-consultation 远端 published 基线已变化（当前版本 %，snapshot %），停止发布 v12。',
  ...
  set
    ...
    source_path = '/Users/apple/vibe coding project/俱乐部官网/supabase/skill-sources/hai-consultation/v12/SKILL.md',
    is_enabled = true,
    updated_at = now()
  where id = v_skill_id;
  ...
  (
    ...
    12000,
    10,
    jsonb_build_object('kind', 'method_index', 'version', 'v12')
  ),
  (
    v_version_id,
    ...
    12000,
    20,
    jsonb_build_object('kind', 'evaluation', 'version', 'v12')
  );
  ...
  if v_reference_count <> 2 then
    raise exception 'hai-consultation v12 reference 数量异常：%', v_reference_count;
  end if;
