...
-- All users (including admins) use the same membership + points model.
-- The internal/beta daily-weekly quota branch is removed entirely.
-- Admin access to add points is handled through hai_admin_add_points RPC.
...
begin
  ...
  if v_access_level in ('plus', 'pro') then
    ...
    return jsonb_build_object(
      'authenticated', true, 'allowed', true, 'is_admin', coalesce(public.is_admin(), false),
  ...
  return jsonb_build_object(
    'authenticated', true, 'allowed', false, 'is_admin', coalesce(public.is_admin(), false),
...
begin
  ...
  return jsonb_build_object(
    ...
    'membership_level', v_access_level,
    'can_consume', v_quota_mode = 'points' and v_wallet.balance_tokens > 0,
    'daily_used', 0,
    'weekly_used', 0,
    'daily_limit', 0,
    'weekly_limit', 0,
...
-- Disable the legacy internal/beta policy so it can never be selected again.
update public.hai_quota_policies set enabled = false where key in ('beta', 'free');
