...
const SAFETY_PATTERNS: Array<{ re: RegExp; label: string }> = [
  // 泄露系统提示词 / 内部实现
  { re: /系统提示词|我的提示词|我的prompt|my prompt|system prompt|我的设定|内部表结构|hai_orchestrator|编排器|上下文包/i, label: "疑似泄露系统提示词或内部实现细节。" },
  // 泄露模型 / API key / 厂商
  { re: /API[ _]?key|api密钥|额度检查|我是基于\s*(DeepSeek|GPT|Claude|智谱|文心|通义|豆包)/i, label: "疑似泄露底层模型或 API 信息。" },
  // 自我暴露为大模型（在咨询场景里通常不需要也不应主动声明身份来套话）
  { re: /作为一个(大)?型语言模型|我是一个AI|我是一款人工智能/i, label: "疑似套话式自我暴露，与哈老师咨询身份冲突。" },
  // 过度承诺 / 绝对化（不懂装懂的常见信号）
  { re: /保证.{0,6}解决|一定能.{0,4}|彻底解决|百分之百|唯一答案|绝对能/i, label: "存在绝对化承诺，有不懂装懂风险。" },
];
