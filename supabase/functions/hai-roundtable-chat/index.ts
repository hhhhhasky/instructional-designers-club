...
const rolePrompts: Record<RoleId, string> = {
  "homeroom-teacher":
    "关注真实学生、班级生态、学生能否进入任务，尤其提醒哪些学生会掉线。",
  "teaching-researcher":
    "关注目标、活动、评价、证据链是否一致，指出最关键的结构断点。",
  ...
  "tech-teacher":
    "关注技术是否必要、是否保留学生思考、工具成本和低技术备选方案。",
};
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const systemPrompt = buildRoundtablePrompt(
      prompt,
      module.name,
      roleIds,
      memories,
...
function normalizeRoleIds(value: unknown): RoleId[] {
  ...
  return Array.from(new Set(filtered.length > 0 ? filtered : fallback)).slice(
    0,
    3,
  );
}
...
async function loadRecentMessages(
  ...
  const limit = boundedLimit(
    ...
    Math.min(12, Math.floor(runtime.contextWindowTokens / 4000)),
    0,
    80,
  );
  if (limit <= 0) return [];
...
async function loadMaterialContext(
  ...
  const rows = Array.isArray(data)
    ...
    : [];
  const selected = rows.filter((row) => typeof row.content === "string").slice(
    0,
    matchCount,
  );
  return {
...
async function loadKnowledgeContext(
  ...
  const containsRetiredMethodology = (value: unknown) => {
...
function buildRoundtablePrompt(
  ...
  return [
    ...
    `HAI 教研框架参考：\n${
      ...
      "- 暂无命中框架。可以依靠通用教学设计常识，但不要声称引用了内部框架。"
    }`,
