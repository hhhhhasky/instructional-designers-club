...
### 9.7 01/02/03 真人口述咨询风格校准（2026-07-13，代码候选，未发布）
...
- 待执行迁移：`supabase/migrations/20260713090000_hai_consultation_voice_prompt_calibration.sql`。

**发布状态**：未执行迁移、未写线上 Prompt 配置、未部署 Edge Function。应先用 Q003/Q004/Q014/Q020/Q021/Q022/Q025/Q029 做旧版 vs 候选版全文回归，确认简单问题不再被强行深层化，公开课问题不再忽略现实评审偏好，再决定是否发布。

### 9.8 01 文档人工反馈驱动的 R10（2026-07-13，本地候选，未发布）
...
详细分析：`docs/HAI_R10_01_FEEDBACK_PROMPT_ITERATION.md`。待执行迁移：`supabase/migrations/20260713113000_hai_r10_personal_methodology_and_voice.sql`。

发布状态：未写线上配置、未部署。下一步先用 Q001、Q004、Q005、Q008、Q009、Q010 做本地回归，重点检查是否命中个人框架、是否完全无 Markdown，以及课程引导是否自然且无编造。

### 9.9 教学通识课结构化方法库 R11.1（2026-07-13，本地候选，未发布）
...
发布状态：未执行 R09、R10、R11 迁移，未写线上配置，未部署 Edge Function。当前已完成本地候选与外部模型冒烟验证；是否上线仍需用户确认。
