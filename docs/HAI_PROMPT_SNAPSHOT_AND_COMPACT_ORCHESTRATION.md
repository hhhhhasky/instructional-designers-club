...
2026-07-13 已执行并登记 `20260713170000` 迁移，`hai-chat` 已部署为 v13。线上配置核验显示，案例、理论、表达检索上限均为 0，用户素材和通用知识库检索均为 false，完整总方法论、教学公理、公式库和独立回答编排器均为 disabled。
...
语义路由是一次独立模型调用，当次实测约 6,054 tokens，主要由 35 张方法卡紧凑目录和路由规则构成。它不会再注入回答生成 prompt，但已被快照机制单独记录；后续如继续降低总调用成本，这是下一个应优化的部分。
